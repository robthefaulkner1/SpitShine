...
Every page ships with: a unique `<title>` + meta description, `<link rel="canonical">`,
`robots` directives, Open Graph / Twitter cards, geo meta, `AutoDetailing`/`LocalBusiness`
JSON-LD (plus `FAQPage` on the FAQ), and a generated `sitemap.xml` + `robots.txt`.

- **Canonical domain** lives in `CONFIG["domain"]` (`build.py`). It feeds canonical tags, the
  sitemap, robots, Open Graph, and schema. Change it in one place and rebuild.
- **Google Search Console verification:** paste the HTML-tag token into
  `CONFIG["google_site_verification"]` and rebuild to add the `<meta>` to every page.
- **Sitemap / robots** are regenerated by `build.py` (`write_seo_files()`) — never edit them by
  hand. Submit `https://<domain>/sitemap.xml` in Search Console.

After deploying, to get indexed: (1) verify the domain in Google Search Console, (2) submit the
sitemap, (3) create/claim a **Google Business Profile** (highest-impact step for local service
SEO), and (4) request indexing for the homepage.
